...
class ORM extends Database {
        ...
        if(Database::$db_type=='mysqli') {
            ...
            foreach (Database::pdo_select($s, $d) as $row) {
                ...
                foreach ($row as $key => $value) {
                  if(method_exists($this, 'hideProperties')) {
                    if(!in_array($key, $this->hideProperties())) {
                      $copy->$key=$value;
                    }
                  } else {
                    $copy->$key=$value;
    ...
    public function order($colLogin, $order) {
        if(Database::$db_type=='mysqli') {
            $this->sql_query .= " ORDER BY $colLogin $order";
        }
        return $this;
    }
    ...
    public function innerJoin($table_name, $join_coloumn1, $join_coloumn2, $another_table='') {
        if(Database::$db_type=='mysqli') {
            $this->sql_query .= " LEFT JOIN ".$table_name." ON ".($another_table!='' ? $another_table.'.'.$join_coloumn1:($table_name=='' ? get_called_class().'.'.$join_coloumn1 : $this->table_name.'.'.$join_coloumn1)).' = '.$table_name.'.'.$join_coloumn2;
        }
        return $this;
    }
    public function rightJoin($table_name, $join_coloumn1, $join_coloumn2, $another_table='') {
        if(Database::$db_type=='mysqli') {
            $this->sql_query .= " RIGHT JOIN ".$table_name." ON ".($another_table!='' ? $another_table.'.'.$join_coloumn1:($table_name=='' ? get_called_class().'.'.$join_coloumn1 : $this->table_name.'.'.$join_coloumn1)).' = '.$table_name.'.'.$join_coloumn2;
        }
        return $this;
    }
    ...
    public function getAllResult($select='')
    {
      if(Database::$db_type=='mysqli') {
          if($this->table_name=='') {
              $this->sql_query = "SELECT ".($select=='' ? '*' : $select)." FROM ".get_called_class().$this->sql_query;
          ...
          foreach($v as $arrK=>$arrV) {
            ...
            if (strpos($method, "map") === 0) {
                ...
                if($relationMetaData->relation=='OneToMany'  && $k == $prop  && $relationMetaData->fetchType=='eager' && !$break) {
                  $arrV->$mapTo = $id;
                  if($relationMetaData->saveRelation) $model->save($arrV);
                }
                if($relationMetaData->relation=='ManyToMany'  && $k == $prop  && $relationMetaData->fetchType=='eager' && !$break) {
                  $toET = $relationMetaData->mapToET;
                  $secondId=$arrV->$toET;
                  if($relationMetaData->saveRelation) $secondId=$model->save($arrV);
